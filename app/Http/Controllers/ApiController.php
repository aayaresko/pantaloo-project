            ...
            switch ($request->input('request')) {
                ...
                case 'wager':
                    ...
                    if ($user->balance < $sum) {
                        if (($user->bonus_balance + $user->balance) >= $sum and $token->slot->is_bonus == 1) {
                            ...
                            if (! bccomp(bcadd($transaction->sum, $transaction->bonus_sum, 5), $sum, 5)) {
                                throw new \Exception('Problem with bonus');
                            }
                        } else {
                            throw new \Exception('Not enogh funds[2]');
                        }
                    } else {
                ...
                case 'result':
                    ...
                    if ($user->bonuses()->first()) {
                        ...
                        if (! $wager_transaction) {
                            throw new \Exception('Wager transaction not found');
                        }
                    } else {

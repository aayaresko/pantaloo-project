    ...
    <table class="table table-striped table-bordered dataTable no-footer datatable" role="grid"
        ...
        @foreach($transactions as $transaction)
            <tr role="row">
                ...
                <td class="">{{$transaction->user->created_at->format(trans('dateformat.date_format'))}}</td>
                ...
                <td>
                    @if($transaction->withdraw_status == 0)
                        <div class="btn-group dropdown">
                            <a type="button" class="btn btn-success waves-effect waves-light btn-xs actTransaction"
                               ...
                               href="{{route('globalAffiliates.approve', $transaction)}}">Approve</a>
                            <button type="button"
                            ...
                            <ul class="dropdown-menu" role="menu">
                                <li><a href="{{route('globalAffiliates.approve', $transaction)}}" class="actTransaction"
                                       data-transaction="{{ $transaction->id }}" data-act="Approve">Aprove</a></li>
                                <li><a href="{{route('globalAffiliates.freeze', $transaction)}}" class="actTransaction"
                    ...
                    @elseif($transaction->withdraw_status == -1)
                        <a href="{{route('globalAffiliates.unfreeze', $transaction)}}"
                           class="btn btn-info waves-effect waves-light btn-xs">Unfreeze</a>
                    @elseif($transaction->withdraw_status == 3)
                        <a href="{{route('globalAffiliates.cancel', $transaction)}}"

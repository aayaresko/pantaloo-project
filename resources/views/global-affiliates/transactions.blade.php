                ...
                <td>
                    @if($transaction->withdraw_status == 0)
                        <div class="btn-group dropdown">
                            <a type="button" class="btn btn-success waves-effect waves-light btn-xs actTransaction" data-transaction = "{{ $transaction->id }}" data-act = "Approve" href="{{route('globalAffiliates.approve', $transaction)}}">Approve</a>
                            ...
                            <ul class="dropdown-menu" role="menu">
                                <li><a href="{{route('globalAffiliates.approve', $transaction)}}" class = "actTransaction" data-transaction = "{{ $transaction->id }}" data-act = "Approve">Aprove</a></li>
                                <li><a href="{{route('globalAffiliates.freeze', $transaction)}}" class = "actTransaction" data-transaction = "{{ $transaction->id }}" data-act = "Freeze">Freeze</a></li>
                            </ul>
                        </div>
                    ...
                    @elseif($transaction->withdraw_status == -1)
                        <a href="{{route('globalAffiliates.unfreeze', $transaction)}}" class="btn btn-info waves-effect waves-light btn-xs">Unfreeze</a>
                    @elseif($transaction->withdraw_status == 3)
                        <a href="{{route('globalAffiliates.cancel', $transaction)}}" class="btn btn-warning waves-effect waves-light btn-xs">Cancel</a>
                    @endif
                </td>
            </tr>
        @endforeach
